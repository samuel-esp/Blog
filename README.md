# Personal Blog 🚀

Welcome to my **Blog**, where I share insights on Cloud Engineering, Platform Engineering and Kubernetes.
Built with [Docusaurus](https://docusaurus.io/).

![GitHub Workflow Status](https://img.shields.io/github/actions/workflow/status/samuel-esp/blog/deploy.yml?label=Build)
![GitHub last commit](https://img.shields.io/github/last-commit/samuel-esp/blog)

---

## 🚀 Getting Started

1. Clone the repository:

   ```bash
   git clone https://github.com/samuel/blog.git
   ```

2. Install dependencies:

   ```bash
   npm install
   ```

3. Start the development server:

   ```bash
   npm start
   ```

---

## 📄 License

This project is licensed under the **MIT License**.
See the [LICENSE](LICENSE) file for details.

---

## 🙏 Acknowledgments

The linter configurations in this project were inspired by the amazing work in the
[GoKubeDownscaler repository](https://github.com/caas-team/GoKubeDownscaler) by the Caas-Team.
