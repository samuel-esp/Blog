...
While Istio offers powerful capabilities to developers, tuning its resource usage can be quite complex.
...
Run targeted stress tests (start at 100 RPS and scale up) to determine the CPU and memory points where latency or errors increase.

In the next sections, you'll learn how to optimize resource usage for both the Istio Ingress Gateway and sidecar proxies.
These
optimizations can be considered as a starting point that you can further adjust based on your specific workload and traffic patterns.
...
Whether you have a shared or dedicated Ingress Gateway, the first thing to check is this environment variable in Istiod:

```bash
PILOT_FILTER_GATEWAY_CLUSTER_CONFIG=true
```

This subtle configuration allows the Ingress Gateway to only receive the configuration relevant to the workloads it serves,
which can significantly reduce the resource usage of the proxy and improve performance.

If this variable is not set, the Ingress Gateway will receive routing configuration for all cluster workloads that
define a VirtualService, DestinationRule, or ServiceEntry—even those not relevant to the gateway.
This can lead to a large amount of configuration being pushed to the proxy, which can lead to increased memory consumption,
especially in larger clusters with many services and complex configurations.

You can compare the before-and-after configuration by running the following command to see how many clusters
(services) the Ingress Gateway is configured to handle:

```bash
istioctl proxy-config clusters <ingress-gateway-pod-name> -n <namespace>
```

Before and after enabling the `PILOT_FILTER_GATEWAY_CLUSTER_CONFIG` variable,
