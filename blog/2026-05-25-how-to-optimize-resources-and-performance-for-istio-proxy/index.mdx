...
This default behavior can lead to a large amount of useless configuration being pushed to the proxy, which can subsequently lead to
increased memory consumption, especially in larger clusters with many services and complex routing configurations.
...
When configuring the Istio Ingress Gateway, it's important to assign appropriate CPU and memory resources because even a small
misconfiguration can cause huge problems.
...
increase memory usage and may lead to OOM kills. If your Ingress Gateway experiences OOM kills while idle,
this is one possible root cause worth investigating, but generally I would always double-check this parameter is set
correctly
...
You must ensure that the `concurrency` value is less than or equal to the CPU limit (in vCPUs) assigned to the proxy.
For example, if you set a CPU limit of 2000m (2 vCPUs), you can set concurrency to 1 or 2.

**Concurrency Rule:**

```math
concurrency\le istio\_proxy\_cpu\_limit\_VCPU\;
```
...
For those wondering, the `Sidecar` resource unfortunately does not apply to Ingress Gateways
as it is designed specifically for sidecar proxies; the Ingress Gateway is treated a special proxy, and it is not affected
...
So, as we have seen above, if you set a CPU limit of 2000m (2 vCPUs), you can set concurrency to 1 or 2.
...
Based on my experience the CPU should be the primary (or even the only) scaling signal of the `istio-proxy`;
memory usage is influenced not only by runtime factors, but also by the amount and complexity of the configuration loaded into the proxy.
My advice regarding memory would be to prefer fine-tuning the requests and limits rather than using memory as a scaling signal
